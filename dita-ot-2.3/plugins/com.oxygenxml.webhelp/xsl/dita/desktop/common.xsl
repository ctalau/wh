...
<xsl:stylesheet 
  ...
  <xsl:variable name="WEBHELP_IS_RESPONSIVE" select="false()"/>
  ...
  <xsl:template name="jsAndCSS">
    ...
    <xsl:if test="$show.changes.and.comments='yes'">
      <xsl:element name="link" namespace="{namespace-uri()}">
        ...
        <xsl:attribute name="type">text/css</xsl:attribute>
        <xsl:attribute name="href">
          <xsl:value-of select="concat($PATH2PROJ, 'oxygen-webhelp/resources/css/p-side-notes.css')"/>
        </xsl:attribute>
        <xsl:comment/>
      </xsl:element>      
    </xsl:if>
    ...
    <xsl:if test="not($WEBHELP_IS_RESPONSIVE)">
  ...
  <xsl:template name="getWHTopicCss">
    <xsl:value-of select="'oxygen-webhelp/resources/css/webhelp_topic.css'"/>
  </xsl:template>
