...
// 				$db->Run($query0);
    ...
    function listForPage($info, $refId = -1, $isForAdmin = false)
    {
        ...
        $paramVersion = $db->sanitize($this->getMinimVersion($product));
            ...
            while ($db->MoveNext()) {
                ...
                if (($db->Field('userId') == $idUser && $db->Field('state') == 'new') || $isForAdmin || $db->Field('state') == 'approved') {
                    ...
                    if ((($idUser == $db->Field('userId') && $db->Field('state') == 'new') && !$isForAdmin) ||
                        ($idUser == $db->Field('userId') && defined('__MODERATE__') && !__MODERATE__) && !$isForAdmin
                    ) {
                        $mustModerate = true;
    ...
    {
        $db = new RecordSet($this->dbConnectionInfo);
        $query = "UPDATE comments SET date=date, visible = 'false' WHERE version < '" . $db->sanitize($minVersion) . "' AND product='" . $db->sanitize($product) . "';";
        $toReturn = true;
        if ($this->isLoggedModerator()) {
            $db->Run($query);
        } else {
        ...
        $query = "UPDATE comments SET date=date, visible = 'true' WHERE version >= '" . $db->sanitize($minVersion) . "' AND product='" . $db->sanitize($product) . "';";
        if ($this->isLoggedModerator()) {
            $db->Run($query);
        } else {
            $toReturn = false;
        ...
        if ($this->isLoggedModerator()) {
            $selectQ = "SELECT value FROM webhelp WHERE parameter='minVisibleVersion' AND product='" . $db->sanitize($product) . "';";
            if ($db->Run($selectQ) > 0) {
                $query = "UPDATE webhelp SET value = '$minVersion' WHERE parameter='minVisibleVersion' AND product='" . $db->sanitize($product) . "'";
                $db->Run($query);
            } else {
                $query = "INSERT INTO webhelp (parameter, value, product) VALUES ('minVisibleVersion', '" . $db->sanitize($minVersion) . "', '" . $db->sanitize($product) . "')";
                $db->Run($query);
            }
        } else {
            $toReturn = false;
